...
const Sidebar = () => {
  ...
  console.log("🚀 ~ file: Sidebar.js:13 ~ Sidebar ~ categories:", categories)
        ...
        <ul className='cat-list'>
          {/* {
            categories.map((category, idx) => {
              console.log("🚀 ~ file: Sidebar.js:31 ~ categories.map ~ idx:", idx)
              return (
          ...
          {
  categories
    .filter((_, idx) => [7, 8,9,10, 11,14 ].includes(idx))
    .map((category, idx) => {
      console.log("🚀 ~ file: Sidebar.js:31 ~ categories.map ~ idx:", idx)
      return (
        <li key={idx} onClick={() => dispatch(setSidebarOff())}>
